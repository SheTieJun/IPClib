...
import static me.shetj.sharedmemory.Helper2.OPEN_READWRITE;
...
public class ServerClientService extends Service {
    ...
    public class RemoteCtrlImpl extends IRemoteCtrl.Stub {
        ...
        public void readFile(String msg) throws RemoteException {
            ...
            if (mParcelFileDescriptor != null) {
                memoryFile = MemoryFileHelper.openMemoryFile(mParcelFileDescriptor, MEMORY_SIZE, OPEN_READWRITE);
            }
            byte[] buffer = (msg+ "服务端返回的实现").getBytes();
            readImage(buffer);
        }

        @Override
        public void linkToDeath(IBinder binder) throws RemoteException {
            Log.d(Utils.TAG, " 服务端 建立链接  ");
            mBinder = binder;
    ...
    IBinder.DeathRecipient mDeathRecipient = new IBinder.DeathRecipient() {
        ...
        public void binderDied() {
            //客户端可以执行释放操作
            Log.d(Utils.TAG, " 调用端已结束，释放调用端的逻辑 ");
            if (mBinder != null) {
